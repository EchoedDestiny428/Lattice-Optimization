...
# Find and sort all available generated samples
...
for index, sample_id in enumerate(sample_ids):
    ...
    # Load metadata JSON to pull dynamic physical properties
    with open(meta_file, "r") as f:
        metadata = json.load(f)

    # DYNAMIC BOX SIZE PARSING (Converts mm to meters safely)
    box_size_mm = float(metadata.get("box_size", 10.0))
    height = box_size_mm / 1000.0  # mm -> meters
    area = height * height  # m^2
    
    # Maintain a constant 1% (0.01) compression strain for testing uniformity
    strain = 0.01  
    disp = strain * height  # m

    voxels = np.load(voxel_file)["voxels"]

    mapdl = None
    try:
        # 2. Build Voxel Mesh using the dynamic height dimension
        ...
        # 6. Calculate Homogenized Properties
        stress = abs(fz) / area
        E_eff = stress / strain

        print(f"-> Successfully Solved. Size: {box_size_mm}mm | E_eff: {E_eff / 1e9:.4f} GPa")

        # Append data dictionary containing inputs and targets
        record = {
            "sample_id": sample_id,
